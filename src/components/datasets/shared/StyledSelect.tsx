    ...
    <div className="w-full">
      {label && (
        <label
          ...
          style={{ color: finalTokens.textPrimary }}
        >
          {label}
        </label>
      )}
      ...
      >
        <SelectTrigger
          ...
          className={`transition-all duration-200 ease-out ${triggerClassName ?? ""}`}
          style={{
        ...
        <SelectContent
          ...
          style={{
            background: isDark
              ? "rgba(26, 34, 64, 0.95)"
              : finalTokens.surfaceCard,
            borderColor: isDark
              ? "rgba(255, 255, 255, 0.15)"
              : finalTokens.borderDefault,
        ...
        >
          {options.map((option) => (
            <SelectItem
              key={option.value}
              value={option.value}
              className="transition-colors duration-150 data-[highlighted]:bg-accent data-[state=checked]:bg-accent/80"
              style={{
                color: finalTokens.textPrimary,
              }}
            >
