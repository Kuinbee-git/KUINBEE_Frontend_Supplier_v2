...
export default function SupplierReviewsPage() {
  ...
  const [selectedDataset, setSelectedDataset] =
    useState<DatasetWithReviews | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setDatasets(await fetchAllSupplierDatasets());
      } catch (error) {
  ...
  const overallAvg = useMemo(() => {
    ...
    return (
      withRatings.reduce((sum, d) => sum + Number(d.rating || 0), 0) /
      withRatings.length
    );
  }, [datasets]);
  ...
  const handleSelectDataset = async (dataset: PublishedDatasetListItem) => {
    try {
      ...
      setSelectedDataset({
        ...dataset,
        reviews: await fetchAllDatasetReviews(dataset.id),
      });
    } catch (error) {
          ...
          {selectedDataset.reviews.length > 0 && (
            <GlassCard className="p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                {/* Average Score */}
                <div
                  className="flex flex-col items-center gap-1 sm:pr-6"
                  style={{ borderRight: `1px solid ${tokens.borderSubtle}` }}
                >
                  <span
                  ...
                  <span
                    ...
                    style={{ color: tokens.textMuted }}
                  >
                    {selectedDataset.reviews.length} review
                    {selectedDataset.reviews.length === 1 ? "" : "s"}
                  </span>
                </div>
                {/* Bars */}
